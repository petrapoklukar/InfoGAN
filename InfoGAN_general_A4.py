...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 11 08:46:42 2020
...
class InfoGAN(nn.Module):
    ...
    def init_sharedDandQ(self):
        """Initialises the shared weights between discriminator and Q."""
        try:
            class_ = getattr(InfoGAN_models, self.config['sharedDandQ_config']['class_name'])
            self.sharedDandQ = class_(self.config['sharedDandQ_config'], self.data_config).to(self.device)
            print(' *- Initialised sharedDandQ: ', self.config['sharedDandQ_config']['class_name'])
        except: 
            raise NotImplementedError(
                    'sharedDandQ class {0} not recognized'.format(
                            self.config['sharedDandQ_config']['class_name']))
    ...
    def init_optimisers(self):
        ...
        if optim_type == 'Adam':
            ...
            self.optimiser_G = optim.Adam(
                    list(self.generator.parameters()) + list(self.Qnet.parameters()) + list(self.sharedDandQ.parameters()),
            ...
            self.optimiser_D = optim.Adam(
                    list(self.sharedDandQ.parameters()) + list(self.Dnet.parameters()), 
...
#        print('===\ngradients:{}'.format(total_norm))        
    ...
    def print_model_params(self):
        ...
        num_dparameters = self.count_parameters(self.sharedDandQ) 
        print_trainable_param(self.sharedDandQ, 'sharedDandQ', num_dparameters)
        self.config['sharedDandQ_config']['n_model_params'] = num_dparameters
        
        num_iparameters = self.count_parameters(self.Qnet) 
        print_trainable_param(self.Qnet, 'Qnet', num_iparameters)
        self.config['Qnet_config']['n_model_params'] = num_iparameters

        num_iparameters = self.count_parameters(self.Dnet) 
        print_trainable_param(self.Dnet, 'Dnet', num_iparameters)
        self.config['Dnet_config']['n_model_params'] = num_iparameters
    ...
    def train_infogan(self, train_dataloader, chpnt_path=''):
        ...
        if chpnt_path: 
            ...
            self.init_generator()
            self.init_sharedDandQ()
            self.init_Qnet()
            self.init_Dnet()
            self.generator.apply(self.init_weights)
            self.sharedDandQ.apply(self.init_weights)
...
#                dinput_noise = torch.empty(real_x.size()).normal_(mean=0, std=0.1)
                ...
                # Loss for real images
                real_pred = self.discriminator_forward(real_x)                
                ...
                fake_pred = self.discriminator_forward(fake_x)
...
#                epochs_d_norms.append(b_d_norms)
                ...
                # Push it through QNet
                gen_x = self.generator((z_noise, dis_noise, con_noise))
                pred_dis_code, pred_con_mean, pred_con_logvar = self.q_forward(gen_x) 
...
#                self.plot_image_grid(gen_x_plotrescale, filename, self.train_dir, n=25)
        ...
        torch.save({
                ...
                'sharedDandQ': self.sharedDandQ.state_dict(), 
    ...
    def save_checkpoint(self, epoch_loss, keep=False):
        ...
        training_dict = {
                ...
                'sharedDandQ_state_dict': self.sharedDandQ.state_dict(),
    ...
    def load_checkpoint(self, path):
        ...
        self.sharedDandQ.load_state_dict(checkpoint['sharedDandQ_state_dict'])
    ...
    def load_model(self, eval_config):
        ...
        # Load the sharedDandQ        
        shareddandq_config = self.config['sharedDandQ_config']
        shareddandq = getattr(InfoGAN_models, shareddandq_config['class_name'])
        self.sharedDandQ = shareddandq(shareddandq_config, data_config).to(self.device)
        shareddandq_model = model_dict['sharedDandQ_']
        if eval_config['load_checkpoint']:
            self.sharedDandQ.load_state_dict(shareddandq_model['model_state_dict'])
            print(' *- Loaded sharedDandQ checkpoint.')
        else:
            self.sharedDandQ.load_state_dict(shareddandq_model)

        # Load the DNet            
        Dnet_config = self.config['Dnet_config']
        Qnet = getattr(InfoGAN_models, Dnet_config['class_name'])
        self.Dnet = Qnet(Dnet_config, data_config).to(self.device)
