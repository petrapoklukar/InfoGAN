...
class FullyConnectedGenerator(nn.Module):
    def __init__(self, config):
        ...
        layer_dims = [self.input_size, 128, 256, 256, 512, self.output_size]
        
        self.generator = nn.Sequential()
        for i in range(1, len(layer_dims) - 1):
            self.generator.add_module('lin' + str(i), nn.Linear(layer_dims[i-1], layer_dims[i]))
            self.generator.add_module('bn' + str(i), nn.BatchNorm1d(layer_dims[i]))
            self.generator.add_module('relu' + str(i), nn.ReLU())
        self.generator.add_module('lin_last', nn.Linear(layer_dims[-2], layer_dims[-1]))
...
class FullyConnectedDiscriminator(nn.Module):
    def __init__(self, config):
        ...
        layer_dims = [self.input_size, 1000, 500, 250, 250, 50]
        
        self.discriminator = nn.Sequential()
        for i in range(1, len(layer_dims) - 1):
            self.discriminator.add_module('lin' + str(i), nn.Linear(layer_dims[i-1], layer_dims[i]))
            self.discriminator.add_module('bn' + str(i), nn.BatchNorm1d(layer_dims[i]))
            self.discriminator.add_module('relu' + str(i), nn.ReLU())
        self.discriminator.add_module('lin_last', nn.Linear(layer_dims[-3], layer_dims[-1]))

        # Output layers
        self.d_out = nn.Sequential(nn.Linear(layer_dims[-1], 1))
        self.categorical_layer = nn.Sequential(nn.Linear(layer_dims[-1], config['n_categorical_codes']), nn.Softmax(dim=-1))
        self.continuous_layer = nn.Sequential(nn.Linear(layer_dims[-1], config['n_continuous_codes']))

    def forward(self, x):
        out = self.discriminator(x)
        validity = self.d_out(out) # Usual discriminator output
        categorical_code = self.categorical_layer(out) # Categorical code
        continuous_code = self.continuous_layer(out) # Continuous code
        return validity, categorical_code, continuous_code
...
class ConvolutionalDiscriminator(nn.Module):
    def __init__(self, config):
        ...
        self.input_size = config['input_size']
        self.output_size = config['output_size']
        convlayer_dims = [self.input_size, 128, 64]

        self.conv = nn.Sequential()
        for i in range(len(convlayer_dims) - 1):
            self.conv.add_module('conv2d' + str(i), nn.Conv2d(
                    convlayer_dims[i], convlayer_dims[i+1], 4, stride=2))
            self.conv.add_module('lrelu' + str(i), nn.LeakyReLU(0.2, inplace=True))
            if i > 0:
                self.conv.add_module('bn', nn.BatchNorm2d(convlayer_dims[i+1]))

        # The height and width of downsampled image
        ds_size = 5

        # Output layers
        self.d_out = nn.Sequential(nn.Linear(convlayer_dims[-1] * ds_size ** 2, 1))
    ...
    def forward(self, x):
        ...
        return validity, categorical_code, continous_code
