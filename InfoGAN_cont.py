...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  5 16:37:17 2019

@author: petrapoklukar

An InfoGAN with only continuous structured latent codes.
...
class InfoGAN(nn.Module):
    ...
    def evaluate(self, data_loader):
        ...
        for i, x in enumerate(data_loader):
            ...
            d_loss, g_loss, i_loss, z_noise, con_noise = self.forward(x)
    ...
    def forward(self, x):
        ...
        z_noise, con_noise = self.noise(batch_size)
        fake_x = self.generator((z_noise, con_noise)).detach()
        ...
        # - INFORMATION LOSS FOR CONTUNOUS LATENT CODES ONLY
        # Sample noise, labels and code as generator input
        z_noise, con_noise = self.noise(batch_size)

        # Push it through QNet
        gen_x = self.generator((z_noise, con_noise))
        _, gen_features = self.discriminator(gen_x) 
        pred_con_mean, pred_con_logvar = self.QNet(gen_features)

        i_loss = self.lambda_con * self.gaussian_loss(
                con_noise, pred_con_mean, pred_con_logvar)
        
        g_loss += i_loss 
        return d_loss, g_loss, i_loss, z_noise, con_noise
        
    def noise(self, batch_size):
        """Generates uninformed noise and structured continuous noise."""
    ...
    def train_infogan(self, train_dataloader, chpnt_path=''):
        ...
        for self.current_epoch in range(self.start_epoch, self.epochs):
            ...
            for i, x in enumerate(train_dataloader):
                ...
                # - INFORMATION LOSS FOR CONTINUOUS CODES ONLY        
                # Sample noise, labels and code as generator input
                z_noise, con_noise = self.noise(batch_size)
